...
/**
 * Generate alerts automatically based on subscriptions and spending patterns
 * This function can be called from anywhere to generate alerts
 */
export async function generateAlerts(
  ...
  for (const sub of subscriptions as Array<{
    ...
    if (sub.nextRenewalDate && sub.amount !== undefined) {
  ...
  for (const tx of thisMonthTxns as Array<{
    merchant?: string;
  ...
  for (const tx of lastMonthTxns as Array<{
    merchant?: string;
    rawDescription?: string;
    amount?: number;
  }>) {
