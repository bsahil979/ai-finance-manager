 ...
 * Handles common Indian bank SMS and UPI app notification formats
 */
function parseSmsOrUpi(text: string): ParsedTransaction | null {
  ...
  // Common patterns: "TO", "FROM", "AT", merchant names after keywords
  ...
  try {
    ...
    if (!text || typeof text !== "string" || text.trim().length === 0) {
      return NextResponse.json(
        { error: "SMS/UPI text is required" },
        { status: 400 },
      );
    }

    // Parse the SMS/UPI text
    const parsed = parseSmsOrUpi(text);

    if (!parsed) {
      return NextResponse.json(
        { error: "Could not parse transaction details from the text. Please check the format." },
        { status: 400 },
      );
    }
    ...
    const transaction = {
      ...
      type: parsed.type,
      category: undefined, // Can be categorized later
      isSubscription: false,
      source: "sms" as const, // Mark as SMS/UPI source
      createdAt: now,
      updatedAt: now,
    };
    ...
    console.error("Error parsing SMS/UPI", error);
    return NextResponse.json(
      { error: "Failed to parse and create transaction" },
      { status: 500 },
    );
  }
}
